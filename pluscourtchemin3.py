...
def Algorithme(event):
    ...
    global listeOuverte, listeFermee, noeudCourant, text, tabCasesListeOuverte, tabCasesListeFermee, intervalTemps
    ...
    while not (noeudCourant.ligne == noeudFinal.ligne and noeudCourant.colonne == noeudFinal.colonne) and listeOuverte != []:
    ...
    elif not listeOuverte:
        # simple animation
        text = menu.create_text(115, 5, text="--- PAS DE SOLUTION ---", anchor=NW, font="Century 10 normal bold",
                                fill='red')
        for i in range(2):
            fen.update()
...
def AjouterCasesAdjacentes(noeudCourant):
    ...
    for direction in deplacements:
        ...
        if 0 <= coordSuivante[0] <= len(tabGrille) - 1 and \
                0 <= coordSuivante[1] <= len(tabGrille[0]) - 1:
                ...
                if not DejaPresentDansListe(noeudTemp, listeFermee):
                    ...
                    if n:
                        ...
                        if noeudTemp.coutG < n.coutG:
                            # Si il est inférieur on remplace les couts et le parent de n par ceux du voisin
                            # récemment trouvé
...
def Menu():
    ...
    menu.delete(ALL)
    menu.create_text(10, 10, text="Menu", anchor=NW, font="Century 10 normal bold", fill='gold')
    ...
    textQuatrePtsCardinaux = menu.create_text(210, 120, text="4 : 4 points cardinaux", anchor=NW,
                                              font="Century 10 normal bold", fill='white')
    textHuitPtsCardinaux = menu.create_text(210, 135, text="8 : 8 points cardinaux", anchor=NW,
                                            font="Century 10 normal bold", fill='red')
    ...
    textRacineDistEucli = menu.create_text(210, 170, text="1 : Racine Dist Euclidienne", anchor=NW,
                                           font="Century 10 normal bold", fill='red')
    textDistEucli = menu.create_text(210, 185, text="2 : Distance Euclidienne", anchor=NW,
                                     font="Century 10 normal bold", fill='white')
    textDistManhattan = menu.create_text(210, 200, text="3 : Distance Manhattan", anchor=NW,
                                         font="Century 10 normal bold", fill='white')
...
menu = Canvas(fen, width=400, height=220, bg='grey')
